...
class ExerciseRecordControllerTest {
    ...
    @BeforeEach
    void setUp() {
        exerciseRecordMapper.delete(null);
        userMapper.delete(null);

        // Main test user
        User user = new User();
        user.setUsername("exercisetest");
        ...
        userId = user.getId();
        token = jwtUtil.generateToken(userId, "exercisetest", "user");
    ...
    void addRecord_success() throws Exception {
        String body = "{\"recordDate\":\"2026-05-12\",\"exerciseType\":\"running\"," +
                "\"duration\":30,\"caloriesBurned\":250,\"intensity\":\"medium\",\"notes\":\"morning run\"}";
    ...
    void addRecord_noAuth_returns401() throws Exception {
        String body = "{\"recordDate\":\"2026-05-12\",\"exerciseType\":\"running\"," +
                "\"duration\":30,\"caloriesBurned\":250}";
    ...
    @Test
    void addRecord_missingType_returns400() throws Exception {
        String body = "{\"recordDate\":\"2026-05-12\"," +
                "\"duration\":30,\"caloriesBurned\":250}";
    ...
    void updateRecord_success() throws Exception {
        ...
        String createBody = "{\"recordDate\":\"2026-05-12\",\"exerciseType\":\"running\"," +
                "\"duration\":30,\"caloriesBurned\":250}";
        ...
        ExerciseRecord record = exerciseRecordMapper.selectList(null).get(0);

        String updateBody = "{\"id\":" + record.getId() + ",\"recordDate\":\"2026-05-12\"," +
                "\"exerciseType\":\"swimming\",\"duration\":45,\"caloriesBurned\":300}";

        mockMvc.perform(put("/exercise")
                        .cookie(new Cookie("diet_token", token))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(updateBody))
                .andExpect(status().isOk())
    ...
    void deleteRecord_wrongUser_returns403() throws Exception {
        ...
        mockMvc.perform(delete("/exercise/" + record.getId())
    ...
    void listByDate_success() throws Exception {
        ...
        mockMvc.perform(get("/exercise/list")
                ...
                .andExpect(jsonPath("$.data[0].exerciseType").value("running"));
    }
}
